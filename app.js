...
(function () {
  ...
  function makeCard(sym) {
    ...
    card.addEventListener("click", () => addRuneToBoard(sym));
    return card;
  }
  ...
  function renderSymbolGrid(category) {
    ...
    if (cat.subcategories && cat.subcategories.length > 0) {
      ...
      subHeader.className = "grid-section-header";
      subHeader.textContent = "Magic Types";
      symbolGrid.appendChild(subHeader);
      ...
      mainHeader.className = "grid-section-header";
      mainHeader.textContent = "Glyphs";
      symbolGrid.appendChild(mainHeader);
    }
  ...
  function updateSelectedInfo() {
    if (selectedRune) {
      ...
      selectedInfo.textContent = "No symbol selected";
...
`;
  ...
  function loadFromJSON(data) {
    if (!data.runes) { alert("Invalid ruins file."); return; }
