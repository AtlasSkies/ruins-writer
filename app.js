(function () {
  ...
  function addRunToBoard(sym) {
    ...
    const el = template.content.cloneNode(true).querySelector(".placed-ruin");
  ...
  function applyTransform(rune) {
    const size = Math.round(BASE_SIZE * ruin.scale);
    ruin.el.style.left      = ruin.x + "px";
    ruin.el.style.top       = ruin.y + "px";
    ruin.el.style.width     = size + "px";
    ruin.el.style.height    = size + "px";
    ruin.el.style.transform = `rotate(${ruin.rotation}deg)`;
  }
  ...
  function selectRune(rune) {
    ...
    selectedRune = rune;
    if (rune) {
      ruin.el.classList.add("selected");
    }
    updateSelectedInfo();
  }
  ...
  function bindRuneInteraction(rune) {
    ...
    el.addEventListener("mousedown", (e) => {
      ...
      dragOffsetX = e.clientX - boardRect.left - ruin.x;
      dragOffsetY = e.clientY - boardRect.top - ruin.y;
    ...
    el.addEventListener("touchstart", (e) => {
      ...
      const boardRect = whiteboard.getBoundingClientRect();
      dragOffsetX = touch.clientX - boardRect.left - ruin.x;
      dragOffsetY = touch.clientY - boardRect.top - ruin.y;
  ...
  btnExportPNG.addEventListener("click", () => {
    ...
    const pending = placedRunes.map(ruin => new Promise(resolve => {
      ...
      img.onload = () => {
        ctx.save();
        const drawSize = Math.round(80 * ruin.scale);
        const cx = ruin.x + drawSize / 2;
        const cy = ruin.y + drawSize / 2;
        ctx.translate(cx, cy);
...
`;
    ...
    placedRunes.forEach(ruin => {
      const size = 80 * ruin.scale;
      const cx = ruin.x + size / 2;
      const cy = ruin.y + size / 2;
      svgContent += `  <image href="${escapeXml(ruin.src)}" 
         x="${ruin.x}" y="${ruin.y}" width="${size}" height="${size}"
         transform="rotate(${ruin.rotation}, ${cx}, ${cy})"
         data-label="${escapeXml(ruin.label)}"/>\n`;
    });
