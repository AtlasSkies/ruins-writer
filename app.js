(function () {
  ...
  function renderExamplesTab() {
    ...
    function getSrc(cat, sub) {
      const c = CATS[cat];
      if (!c) return null;
      const found = c.subcategories.find(s => s.id.endsWith("_" + sub) || s.id.endsWith(sub));
      return found ? found.src : null;
    }
    function getConnSrc(name) {
      const c = CATS.transformations;
      if (!c) return null;
      const found = c.symbols.find(s => s.id === "tr_" + name || s.label.toLowerCase().replace(/ /g,"_") === name);
      return found ? found.src : null;
    }
    ...
    const examples = [
      {
        ...
        steps: [
          { src: getSrc("life","revival"),  label: "Life — Revival",   note: "Reanimates the physical body." },
          { src: getConnSrc("channel"),     label: "Channel",          note: "Connects the two workings.", connector: true },
          { src: getSrc("memory","mind"),   label: "Memory — Mind",    note: "Targets the consciousness." },
          { src: getConnSrc("channel"),     label: "Channel",          note: "Passes control through.", connector: true },
          { src: getSrc("memory","control"),label: "Memory — Control", note: "Overrides autonomous will." },
        ]
      },
      {
        name: "Puppeteer",
        ...
        steps: [
          { src: getSrc("memory","control"),   label: "Memory — Control",    note: "Overrides the target's will." },
          { src: getConnSrc("channel"),         label: "Channel",             note: "Routes the effect.", connector: true },
          { src: getSrc("space","expansion"),   label: "Space — Expansion",   note: "Extends the effect across distance." },
          { src: getConnSrc("contain"),         label: "Contain",             note: "Limits the effect to one target.", connector: true },
          { src: getSrc("memory","imprint"),    label: "Memory — Imprint",    note: "Writes the controller's commands in." },
        ]
      },
      {
        name: "Farsight",
        ...
        steps: [
          { src: getSrc("light","sight"),       label: "Light — Sight",       note: "Activates the visual channel." },
          { src: getConnSrc("channel"),          label: "Channel",             note: "Flows into spatial extension.", connector: true },
          { src: getSrc("space","expansion"),    label: "Space — Expansion",   note: "Extends the range of perception." },
          { src: getConnSrc("amplify"),          label: "Amplify",             note: "Increases the magnitude of reach.", connector: true },
          { src: getSrc("knowledge","truth"),    label: "Knowledge — Truth",   note: "Strips illusion from what is seen." },
        ]
      },
    ];
    ...
    examples.forEach(ex => {
      ...
      ex.steps.forEach(step => {
        ...
        if (step.src) {
          const img = document.createElement("img");
          img.src = step.src;
          img.className = "example-step-img";
          row.appendChild(img);
        }
