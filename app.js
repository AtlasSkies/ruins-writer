...
(function () {
  ...
  function renderSymbolGrid(category) {
    ...
    if (cat.categorySigil) {
      ...
      sigilWrap.addEventListener("mouseenter", () => showPopup(cat.categorySigil, sigilWrap));
      sigilWrap.addEventListener("mouseleave", hidePopup);
    }
  ...
  function positionPopup(cardEl) {
    ...
    // Place to the right of the panel
    let left = panelRect.right + 8;

    // If off-screen right, place to the left
    if (left + popupW > window.innerWidth) {
      left = panelRect.left - popupW - 8;
    }
